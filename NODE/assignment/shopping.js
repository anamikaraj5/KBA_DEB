...
function AddProd(id,name,price,quantity) 
{
  if (cart.has(id)) 
  {
...
function UpdateQuant(id,quantity) 
{
  if (cart.has(id)) 
  {
          ...
          rl.question("Enter Product Name: ", (name) => {
            rl.question("Enter Product Price: ", (price) => {
              rl.question("Enter Quantity: ", (quantity) => {
                AddProd(id, name, parseFloat(price), parseInt(quantity))
                showMenu()
              })
            })
          })
      ...
      case "2":
        rl.question("Enter Product ID to update: ", (id) => {
          rl.question("Enter new Quantity: ", (quantity) => {
            UpdateQuant(id, parseInt(quantity))
            showMenu()
          })
        })
        break
